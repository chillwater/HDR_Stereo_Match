...
for i=1:folderCount   %scenes folders  (ex.Aloe-2views-3illum-3exp)
   ...
   cd('CostFilter'); % change into CostFilter directory
        ...
        %set file name
	    FileNamet=strcat(subfolderList(1), '_CostFilter_BadPixelRate');
        FileName=FileNamet{1}
        display_legend = 1; %dont display legend
	    matlab_createBar_BadPixelRate;       %run script to create plot
   cd('..');
   cd('SemiGlob'); % change into SemiGlob directory
	    BarTitle=strcat(subfolderList(1),' - SemiGlob')
        %set file name
	    FileNamet=strcat(subfolderList(1), '_SemiGlob_BadPixelRate');
        FileName=FileNamet{1}
        display_legend = 1; %dont display legend
	    matlab_createBar_BadPixelRate;                     %run script to create plot
   cd('..');
   cd('SSMP'); % change into SSMP directory
        ...
        %set file name
	    FileNamet=strcat(subfolderList(1), '_SSMP_BadPixelRate');
        FileName=FileNamet{1}
        display_legend = 1; %dont display legend
	    matlab_createBar_BadPixelRate;              %run script to create plot
